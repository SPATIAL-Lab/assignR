...
pdRaster.default = function(r, unknown, prior = NULL, mask = NULL, 
  ...
  errorV = values(rescaled.sd)
  meanV = values(rescaled.mean)
  result = NULL
  temp = list()
...
pdRaster.isoStack = function(r, unknown, prior = NULL, mask = NULL, 
  ...
  for(i in r){
    if(inherits(i, "SpatRaster")){
      ...
      if(nlyr(i) != 2) {
        stop("r should contain SpatRasters with two layers 
         (mean and standard deviation)")
      }
    } else{
      stop("r should contain SpatRasters with two layers 
  ...
  for(i in 2:ni){
    ...
    meanV = cbind(meanV, values(rescaled.mean))
    errorV = cbind(errorV, values(rescaled.sd))
  }
...
check_mask = function(mask, r){
  ...
  if (!is.null(mask)) {
    if(inherits(mask, "SpatialPolygons")){
      ...
      if(proj4string(mask) != crs(r[[1]], proj = TRUE)) {
