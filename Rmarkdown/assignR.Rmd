...
install.packages("assignR", repos = "https://cloud.r-project.org")
library(assignR)
```
...
Now lets look at a list of species names and the number of samples available for them. We won't show the result here since it's a long table, but here's the code:

```{r knownOrig_taxa, results="hide"}
View(summary(knownOrig$Taxon))
```
...
Here you can see that even though Utah is quite a bit smaller the isotopic evidence suggests it's much more likley to be the origin of each sample. This result is consistent with what you might infer from a first-order comparison of the state map with the posterior probability maps, above.
...
```{r QA1, warning=FALSE}
qa1 = QA(d2h_world, d, valiStation = 10, valiTime = 5, mask = naMap, name = "normal")
```
...
The first three panels show three metrics, granularity (higher is better), bias (closer to 1:1 is better), and sensitivity (higher is better). The second plot shows the poterior probabilities at the known locations of origin relative to random (=1, higher is better). More information is provided in Ma et al., in review.
...
qa2 = QA(d2h_fuzzy, d, valiStation = 10, valiTime = 5, mask = naMap, name = "fuzzy")
```
