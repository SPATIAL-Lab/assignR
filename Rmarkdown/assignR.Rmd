...
```{r QA_unstable}
plot.QA(QA(d2h_world, subOrigData(taxon = "Buteo platypterus"), valiStation = 6, valiTime = 10, mask= naMap, setSeed = 10))
```

Although the isotope-based assignments still have substantial power, notice the low accuracy values at probability quantiles above 0.4. The plot indicates that at the 0.8 probability quantile, where you would expect to correctly assign 80% of samples, the method is only correctly assigning ~60%. It turns out that the sample set for *Buteo platypterus* is relatively small and the relationship with the isoscape values somewhat non-linear:
