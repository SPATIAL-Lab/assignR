...
```{r devtools, message=FALSE, warning=FALSE, results="hide"}
...
```{r QA_unstable, warning=FALSE}
d2 = subOrigData(taxon = "Buteo platypterus")
plot.QA(QA(d2h_world, d2, valiStation = 5, valiTime = 5, mask= naMap))
```

Although the isotope-based assignments still have substantial power, notice the low accuracy values. The plot indicates that at the 0.8 probability quantile, where you would expect to correctly assign 60% of samples, the method is correctly assigning less than 60%. It turns out that the sample set for *Buteo platypterus* is relatively small and the relationship with the isoscape values somewhat non-linear:

```{r Buteo}
plot(extract(d2h_world[[1]], d2), d2$d2H, xlab = "Isoscape", ylab = "Sample")
```
